(function($) {
	...
	window.partner = $.fn.partner = {
		displayGraph: function(opt) {
			...
			var svg = d3.select("#" + options.posId).append("svg")
				.attr("width", width)
				.attr("height", height);
			...
			var node = svg.selectAll(".node")
				...
				.on("click", function(d) {
					if (d === root || d.enterpriseName === "未知企业") {
						return;
					}
					options.recur();
				})
...
jQuery(document).ready(function($) {
	...
	// 显示气泡图
	partner.displayGraph({
		list: list,
		posId: "graphDisplay",
		recur: function() {
			$.jBox.tip("请求正在处理!请稍后......", 'loading');
			$.ajax({
				url: basePath + "enterprise/showEnterprisePartnerChart?" + new Date().getTime(),
				dataType: "json",
				type: "get",
				async: false,
				data: {
